...
if old not in s:
    raise SystemExit('expected v4 imports not found')
s=s.replace(old,new,1)

start=s.index('async function setupFaceModel(){')
end=s.index('async function setupPhoneModel(){', start)
face_fn="""async function setupFaceModel(){
  if(faceLandmarker)return;
  els.face.textContent='LOADING';
  els.faceDetail.textContent='Loading face runtime…';
  if(!mpLib){
    mpLib=await import('https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.21/+esm');
  }
  ...
  const opts={
    baseOptions:{...base,delegate:'GPU'}, runningMode:'VIDEO', numFaces:1,
    minFaceDetectionConfidence:0.35,minFacePresenceConfidence:0.35,minTrackingConfidence:0.35,
  ...
  try{faceLandmarker=await FaceLandmarker.createFromOptions(vision,opts);}
    ...
    console.warn('GPU face runtime failed; retrying CPU',err);
    faceLandmarker=await FaceLandmarker.createFromOptions(vision,{...opts,baseOptions:{...base,delegate:'CPU'}});
  }
  els.face.textContent='MODEL READY';
  els.faceDetail.textContent='HF face model loaded';
}

"""
s=s[:start]+face_fn+s[end:]

start=s.index('async function setupPhoneModel(){')
end=s.index('async function requestWakeLock(){', start)
phone_fn="""async function setupPhoneModel(){
  ...
  try{
    if(!hfLib){
      hfLib=await import('https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1');
      hfLib.env.allowLocalModels=false;hfLib.env.useBrowserCache=true;
    }
    ...
    console.error(err);els.phone.textContent='OFFLINE';els.phoneDetail.textContent=`HF phone AI: ${err?.message||'load failed'}`;
  }finally{phoneModelLoading=false;}
}

"""
s=s[:start]+phone_fn+s[end:]
...
"els.cameraMessage.textContent='START FAILED — see error below';\n    els.face.textContent='ERROR';\n    els.faceDetail.textContent=err?.message||'Face AI failed';\n    els.reason.textContent=err?.message||'Camera/Hugging Face model error';")

s=s.replace("if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=4').catch(()=>{});",
"window.addEventListener('error',e=>{ if(!running){ els.reason.textContent='App error: '+(e.message||'unknown'); els.faceDetail.textContent=e.message||'App error'; } });\nwindow.addEventListener('unhandledrejection',e=>{ if(!running){ const m=e.reason?.message||String(e.reason||'unknown'); els.reason.textContent='Load error: '+m; els.faceDetail.textContent=m; } });\nif('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=5').catch(()=>{});")
app.write_text(s)

index=Path('driver-guard/index.html')
i=index.read_text().replace('styles.css?v=4','styles.css?v=5').replace('app.js?v=4','app.js?v=5')
index.write_text(i)

sw=Path('driver-guard/sw.js')
w=sw.read_text().replace('driver-guard-v4','driver-guard-v5').replace("'./styles.css?v=4','./app.js?v=4','./detection-core.js?v=4'","'./styles.css?v=5','./app.js?v=5','./detection-core.js?v=5'")
sw.write_text(w)
...
if "import { pipeline" in s or "import { FaceLandmarker" in s: raise SystemExit('external static import still present')
